...
describe QueueItemsController do
  ...
  describe "Post create" do
   ...
   it 'does not add video to the  queue if the video is alredy in the queue' do
      alice = Fabricate(:user)
      session[:user_id] = alice.id
      monk = Fabricate(:video)
      Fabricate(:queue_item, video: monk, user: alice)
    ...
    it 'redirects tj the sign in page for unauthenticated user' do
  ...
  describe "DELETE destroy" do
    ...
    it "does not delete the queue item id the queue item is not in the current user's queue" do
      alice = Fabricate(:user)
      bob = Fabricate(:user)
      session[:user_id] = alice.id
    ...
    it "redirecrts to sign in page for unauthenticated user" do 
      delete :destroy, id: 1
      expect(response).to redirect_to sign_in_path
    end
  ...
  describe "POST update_queue" do
    ...
    context "with invalid inputs" do
      ...
      it "redirects to the my queue path" do
        ...
        queue_item1 = Fabricate(:queue_item, user: user, position: 1)
        queue_item2 = Fabricate(:queue_item, user: user, position: 2)
        post :update_queue, queue_items: [{id: queue_item1.id, position: 3.6 },{id: queue_item2.id, position: 2}]
        expect(response).to redirect_to my_queue_path
      end
    ...
    context "with queue items that do not belong to the current user" do
      it "does not change queue item that does not belong to current user " do
        alice = Fabricate(:user)
        bob = Fabricate(:user)
